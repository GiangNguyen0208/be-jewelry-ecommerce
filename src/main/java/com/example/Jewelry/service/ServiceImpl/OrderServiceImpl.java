...
public class OrderServiceImpl implements OrderService {
    ...
    @Override
    @Transactional
    public CommonApiResponse createOrder(OrderRequestDTO dto) {
        try {
            if (dto.getCartItems() == null || dto.getCartItems().isEmpty()) {
                return CommonApiResponse.fail("Giỏ hàng rỗng");
            }
            ...
            return CommonApiResponse.success("Tạo đơn hàng thành công, vui lòng kiểm tra email để xác thực thanh toán bằng OTP.");
        } catch (Exception e) {
            return CommonApiResponse.fail("Lỗi tạo đơn hàng: " + e.getMessage());
        }
    }
